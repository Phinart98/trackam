...
<template>
  <div class="px-4 pt-4 pb-6 lg:px-6 lg:pt-6 max-w-2xl lg:mx-0">
    ...
    <div class="flex items-center justify-between mb-5">
      <div>
        <h1 class="text-xl font-bold text-slate-900 font-display lg:text-2xl">Savings Goals</h1>
        <p class="text-sm text-slate-400 mt-0.5">Save toward what matters — school fees, stock, a new stall</p>
      </div>
      <button
        v-if="!showForm"
    ...
    >
      <div v-if="showForm" class="bg-white rounded-xl border border-slate-200 p-4 mb-5 space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-sm font-bold text-slate-800">{{ isEditing ? 'Edit Goal' : 'New Goal' }}</h2>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-1.5">
            <button
              ...
              @click="formIcon = icon"
            >
              <UIcon :name="icon" class="text-base" :class="formIcon === icon ? formColor.color : 'text-slate-500'" />
            </button>
          </div>
        </div>
    ...
    <!-- Add / Withdraw Funds Modal -->
    <div v-if="fundingGoalId" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30" @click.self="fundingGoalId = null">
      <div class="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm p-5 space-y-4 safe-area-bottom">
        <h3 class="text-sm font-bold text-slate-800">{{ isAdding ? 'Add to savings' : 'Withdraw from savings' }}</h3>
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-semibold">{{ auth.currency }}</span>
          <input
            v-model="fundAmount"
        ...
        <div class="flex gap-2">
          <button class="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm" @click="fundingGoalId = null">Cancel</button>
          <button
    ...
    <!-- Goals List -->
    <div v-if="goalStore.goals.length > 0" class="space-y-3">
      <div
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div class="relative w-14 h-14 shrink-0">
            <svg class="w-full h-full -rotate-90" viewBox="0 0 60 60">
              <circle cx="30" cy="30" r="26" fill="none" stroke-width="4" class="stroke-slate-100" />
              <circle
                cx="30" cy="30" r="26" fill="none" stroke-width="4" stroke-linecap="round"
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <span v-if="(goalStore.progressMap[goal.id] ?? 0) >= 100" class="text-lg">🎉</span>
              <UIcon v-else :name="goal.icon" class="text-lg" :class="goal.color" />
            </div>
          </div>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <p class="text-sm font-bold text-slate-800 truncate">{{ goal.name }}</p>
              <span v-if="(goalStore.progressMap[goal.id] ?? 0) >= 100" class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 shrink-0">
                Done!
              </span>
            </div>
            <div class="flex items-baseline gap-1 mb-2">
              <span class="text-sm font-bold" :class="goal.color">{{ formatCurrency(goal.currentAmount, goal.currency) }}</span>
        ...
        <div class="flex items-center gap-2 mt-3 pt-3 border-t border-slate-50">
          ...
          <button class="p-2 text-slate-400 hover:text-slate-600" :aria-label="`Edit ${goal.name}`" @click="startEdit(goal)">
            <UIcon name="i-lucide-pencil" class="text-sm" aria-hidden="true" />
          </button>
          <button class="p-2 text-slate-400 hover:text-red-500" :aria-label="`Delete ${goal.name}`" @click="handleDelete(goal.id)">
    ...
    <div v-else class="py-12 text-center">
      <div class="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center mx-auto mb-4">
        <UIcon name="i-lucide-target" class="text-emerald-500 text-2xl" />
      </div>
      <h3 class="text-base font-bold text-slate-800 mb-1">No goals yet</h3>
